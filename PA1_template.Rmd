...
```{r}
hist(totalStepsPerDay, main ="Frequency Distribution of Total Steps Per Day", xlab="Steps Per Day")
```
...
```{r}
dfmerge <- merge(dfActivity, avg_Steps_by_Interval, by.x = "interval", by.y= "interval")
dfmerge$steps[is.na(dfmerge$steps) ] <- dfmerge$average_steps[is.na(dfmerge$steps) ]
```

### Make a histogram of the total number of steps taken each day and Calculate and report the mean and median total number of steps taken per day. Do these values differ from the estimates from the first part of the assignment? 
```{r}
totalStepsPerDayImputed <- tapply(dfmerge$steps, dfmerge$date, sum)

hist(totalStepsPerDayImputed, main ="Frequency Distribution of Total Steps Per Day", xlab="Steps Per Day")
```
